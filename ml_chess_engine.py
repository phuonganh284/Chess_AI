...
DEFAULT_CHECKPOINT = os.getenv(
    "ML_CHECKPOINT",
    os.path.join(os.path.dirname(__file__), "ml_model.pt"),
)
...
def board_from(value):
    if isinstance(value, chess.Board):
        return value
    if hasattr(value, "board") and isinstance(value.board, chess.Board):
        return value.board
    raise TypeError("Expected a python-chess Board or a GameState with a .board attribute.")


def get_or_create_model(checkpoint_path=DEFAULT_CHECKPOINT):
    global _CACHED_MODEL

    if _CACHED_MODEL is not None:
        return _CACHED_MODEL

    _CACHED_MODEL = load_model(checkpoint_path)
    if _CACHED_MODEL is None:
        print(
            f"No ML checkpoint found at {checkpoint_path}; using an untrained model. "
            "Run `python3 ml_chess_engine.py` to train one.",
            flush=True,
        )
...
def get_or_train_model(checkpoint_path=DEFAULT_CHECKPOINT, force_retrain=False):
    global _CACHED_MODEL
...
def ml_move(
    ...
    if model is None:
        if train_if_missing:
            model = train_model(checkpoint_path=checkpoint_path)
        else:
            model = get_or_create_model(checkpoint_path)

    move, _, _ = mcts_move(board, model, simulations=simulations, training=False)
    return move


def find_best_move(board, depth=None, use_ml_eval=True, simulations=120, model=None):
    del depth, use_ml_eval
    return ml_move(board, model=model, simulations=simulations)
...
def main():
    train_model(
        games=int_from_env("ML_TRAIN_GAMES", 96),
        simulations=int_from_env("ML_TRAIN_SIMULATIONS", 200),
        epochs=int_from_env("ML_TRAIN_EPOCHS", 5),
        checkpoint_path=DEFAULT_CHECKPOINT,
        force_retrain=bool_from_env("ML_FORCE_RETRAIN"),
        resume_training=bool_from_env("ML_RESUME_TRAINING", True),
    )
